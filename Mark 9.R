...
#dev.off()
# # Fit the Cox model
# coxModel <- coxph(survivalObj ~ dataset$flc.grp + dataset$flc.grp, data = dataset)
# 
# # Get the survival probabilities for each level of flc.grp
# survProb <- survfit(coxModel, newdata = list(flc.grp = unique(dataset$flc.grp)))
# 
# # Plot the survival probabilities
# plot(survProb, col = 1:length(unique(dataset$flc.grp)), lty = 1, lwd = 2)
# legend("topright", legend = levels(dataset$flc.grp), col = 1:length(unique(dataset$flc.grp)), lty = 1, lwd = 2)
...
for (i in 1:nrow(dataset)) {
  ...
  for(j in 1:10)
  {
    ...
    survivalTimes <- survivalTimesList[[j]]
    survivalProbs <- survivalProbsList[[j]]
    ...
    probsAvg <- probsAvg + survivalProbs[maxSurvivalTimeIndex]
  }
  observedProbs[i] <- probsAvg/10
}
...
#weights <- (dataset$weights - min(dataset$weights)) / (max(dataset$weights) - min(dataset$weights))
...
for (i in 1:length(folds)) {
  ...
  pred <- predict(bnIPWC, node = "observedDeaths", data = test_data, method = "exact")
...
#bic_score <- bnlearn::score(dag, data = oldData, type = "gaussian")
#print(bic_score)
